...
- **Price caveats:** a card can carry a per-card warning — shown to visitors next to the price — that its fair value is unreliable because the market itself is distorted (tournament demand or deliberate manipulation). Display-only: unlike a price exclusion it does **not** change the number or drop the card from training. Preset reasons and the rendered sentence live in `src/logic/warningText.js` (shared with the prerender so the crawler and the visitor see the same note); hand-maintained data in `src/data/price-warnings.json`, set on the admin card editor below.

### Admin area (hidden, `/admin`)

A password-gated hub (`src/logic/adminGate.ts`) that pulls the editing tools together. The gate is **obfuscation, not security** — the admin code ships to every browser and the check is bypassable — which is fine on purpose: the admin pages only edit this browser's `localStorage` and export JSON, with no server-side action and no private data to reach. The area is `noindex` and absent from the sitemap. Password is stored as a SHA-256 hash in `src/data/admin-gate.json` (never the plaintext).

- **Browse & edit:** the normal site, but with the area unlocked every card page grows an **Admin edit** panel (lazy-loaded, `AdminCardControls`) for that card's artwork rating, trend-price correction and price warning — writing to the same stores the standalone pages use. Edits are captured for the next rebuild; prices don't recompute in the browser.
- **Export all:** one JSON bundling artwork ratings, Tera tags, price exclusions and price warnings — the single file to send back after a session.
- **Tera ex tagging (`/admin/tera`):** mark which Scarlet & Violet ex cards are Tera; see below.
- **Artwork rating (`/admin/artwork`):** Rate illustration quality (10/9/8/worse) on chase cards whose artwork is genuinely their own — illustration, special illustration, secret and full art rares. Double Rares and Ultra Rares are excluded (standard frame, nothing to judge), and so are Promos (see "Why the promo sets aren't displayed" below). **This is a live model factor**, fed from `src/data/artwork-ratings.json`.
